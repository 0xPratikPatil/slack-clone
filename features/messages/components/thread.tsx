...
import { Message } from '@/components/echo/message';
import { Button } from "@/components/ui/button";
import { useGetCurrentMember } from "@/features/members/api/use-current-member";
import { useCreateMessage } from "@/features/messages/api/use-create-message";
import { useGetMessageById } from "@/features/messages/api/use-get-message";
import { useGetMessages } from '@/features/messages/api/use-get-messages';
import { useGenerateUploadUrl } from '@/features/upload/api/use-generate-upload-url';
import { useChannelId } from '@/hooks/use-channel-id';
import { useWorkspaceId } from '@/hooks/use-workspace-id';
import { EchoChannel, EchoMessage, EchoWorkspace } from '@prisma/client';

const Editor = dynamic(() => import('@/components/echo/editor'), {
  ssr: false,
  loading: () => (
...
type CreateMessageValues = {
  ...
  parentMessageId: EchoMessage["id"]
  body: string;
  image?: string
};
...
export const Thread = ({ messageId, onClose }: ThreadProps) => {
  ...
  const [editingId, setEditingId] = useState<EchoMessage["id"] | null>(null);
  ...
  const { data: message, isLoading: isMessageLoading } = useGetMessageById({ messageId });

  const { mutate: createMessage } = useCreateMessage();
  const { mutate: generateUploadUrl } = useGenerateUploadUrl();
  // const { data: results, status, loadMore } = useGetMessages({
  //   channelId,
    ...
    <div className="flex h-full flex-col">
      ...
      <div className="messages-scrollbar flex flex-1 flex-col-reverse overflow-y-auto pb-4">
        ...
        <Message
          ...
          authorName={message.user.name}
          authorImage={message.user.image}
          isAuthor={message.memberId === currentMember?.data?.id}
          body={message.body}
          image={message.image}
          createdAt={message._creationTime}
          updatedAt={message.updatedAt}
